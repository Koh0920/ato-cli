...
pub async fn execute(args: OpenArgs) -> Result<()> {
    ...
    let target_name = target.file_name().and_then(|n| n.to_str()).map(|s| s.to_string());

    if target.extension().map(|e| e.to_string_lossy().to_lowercase()) == Some("capsule".to_string()) {
...
async fn execute_capsule_file(args: &OpenArgs, capsule_path: &PathBuf) -> Result<()> {
    args.reporter
        .notify(format!("📦 Extracting capsule archive: {}", capsule_path.display()))
        .await?;

    let extract_dir = capsule_path
        .parent()
        .map(|p| p.join(format!("{}-extracted", capsule_path.file_stem().unwrap().to_string_lossy())))
        .context("Failed to determine extraction directory")?;

    if extract_dir.exists() {
        args.reporter
            .warn(format!("⚠️  Removing existing extracted directory: {}", extract_dir.display()))
            .await?;
    ...
    args.reporter
        .notify(format!("🚀 Running extracted capsule from: {}", extract_dir.display()))
        .await?;
...
async fn copy_source_files(original_dir: &Path, extract_dir: &Path, reporter: &Arc<CliReporter>) -> Result<()> {
    let entries = fs::read_dir(original_dir)
        .with_context(|| format!("Failed to read original directory: {}", original_dir.display()))?;
    ...
    for entry in entries {
        ...
        if file_name == "source" && path.is_dir() {
            ...
            fs::copy(&path, &dest)?;
            reporter.notify(format!("   ✅ Copied {}", file_name.to_string_lossy())).await?;
...
fn check_has_source_files(dir: &Path) -> bool {
    ...
    for entry in entries {
        if let Ok(entry) = entry {
            ...
            if file_name == "capsule.toml" || file_name == "capsule.lock" || file_name == "config.json" || file_name == "signature.json" {
                continue;
            }

            if path.is_file() {
                let name = file_name.to_string_lossy();
                if name == "package.json" || name == "pyproject.toml" || name == "requirements.txt" || name == "go.mod" || name == "Cargo.toml" {
                    return true;
                }
            ...
            if path.is_dir() && !is_hidden(&file_name) {
                if path.join("package.json").exists() || path.join("pyproject.toml").exists() || path.join("index.js").exists() || path.join("main.py").exists() {
                    return true;
                }
            }
        }
...
fn is_source_file(file_name: &std::ffi::OsString) -> bool {
    let exts = ["js", "ts", "py", "go", "rs", "json", "html", "css", "mjs", "cjs"];
